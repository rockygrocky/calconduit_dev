	...
	public function render_content() {
		...
		<div class="options-control-inputs">
			<label>
				...
				<select name="op-options[<?php linethemes_esc_attr( $this->id ) ?>]" id="<?php linethemes_esc_attr( $name ) ?>">
					<?php
					foreach ( $choices as $value => $label ):
						if ( is_array( $label ) && isset( $label['label'] ) && isset( $label['items'] ) && is_array( $label['items'] ) ): ?>
							<optgroup label="<?php linethemes_esc_attr( $label['label'] ) ?>">';

							<?php foreach( $label['items'] as $item_value => $item_text ): ?>

								<option value="<?php linethemes_esc_attr( $item_value ) ?>"<?php selected( $this->value(), $item_value ) ?>><?php linethemes_esc_html( $item_text ) ?></option>
						...
						<option value="<?php linethemes_esc_attr( $value ) ?>" <?php selected( $this->value(), $value ) ?>><?php linethemes_esc_html( $label ) ?></option>
					<?php endforeach ?>
				</select>
			</label>
		</div><?php
