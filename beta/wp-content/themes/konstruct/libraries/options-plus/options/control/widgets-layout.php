	...
	public function render_content() {
		...
		<div class="options-control-inputs">
			<div class="columns-count">
				...
				<label>
					<input type="radio" name="<?php linethemes_esc_attr( $name ) ?>" value="<?php linethemes_esc_attr( $index ) ?>" <?php checked( $columns_count, $index ) ?> />
			...
			<div class="options-control-layouts">
				<?php foreach ( range(0, $max - 1) as $index ):
					...
					?>
					<div id="<?php linethemes_esc_attr( $name . '-' . $index ) ?>" class="<?php if ( $index == $columns_count ) echo 'active' ?>">
						<div class="widgetslayout-row">
							<?php foreach ( $columns as $width ): ?>
							<div class="widgetslayout-column" data-width="<?php linethemes_esc_attr( $width ) ?>">
								<span><?php linethemes_esc_html( $width ) ?></span>
							</div>
							<?php endforeach ?>
						</div>
					</div>
		...
		<input type="hidden" name="op-options[<?php linethemes_esc_attr( $this->id ) ?>]" value="<?php linethemes_esc_attr( json_encode( $values ) ) ?>" />
		<?php
	}
}
