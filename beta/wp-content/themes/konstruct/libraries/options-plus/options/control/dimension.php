	...
	public function render_content() {
		...
		<div class="options-control-inputs">
			<?php foreach ( $fields as $id => $title ): ?>
				<label for="<?php linethemes_esc_attr( $name . '_' . $id ) ?>">
					<span><?php linethemes_esc_html( $title ) ?></span>
					<input type="text" name="op-options[<?php linethemes_esc_attr( $this->id ) ?>][]"
						   value="<?php linethemes_esc_attr( $values[$field_index++] ) ?>"
						   id="<?php linethemes_esc_attr( $name . '_' . $id ) ?>" />
				</label>
			<?php endforeach ?>
		</div>
		<?php
