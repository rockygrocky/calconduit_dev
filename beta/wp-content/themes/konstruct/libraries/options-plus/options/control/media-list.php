...
class MediaList extends \OptionsPlus\Options\Control
{
	...
	public function render_content() {
		...
		if ( in_array( 'image', $this->media_types ) )
			$buttons[] = sprintf( '<a href="#" class="add-image">%s</a>', esc_html__( 'Insert Image(s)', 'options-plus' ) );

		if ( in_array( 'video', $this->media_types ) )
			$buttons[] = sprintf( '<a href="#" class="add-video">%s</a>', esc_html__( 'Insert Video(s)', 'options-plus' ) );
		?>
			...
			<input type="hidden" name="op-options[<?php linethemes_esc_attr( $this->id ) ?>]" value="<?php linethemes_esc_attr( json_encode( $value ) ) ?>" />
	...
	public static function templates() {
		$videoPane = new \OptionsPlus\Options\Container( array(
			...
			'sections'  => array(
				'all' => array(
					'title' => esc_html__( 'All', 'options-plus' )
				) 
			),
			'controls'  => array(
				'url' => array(
					'type'    => 'media-input',
					'section' => 'all',
					'label'   => esc_html__( 'URL', 'options-plus' ),
					'library' => 'video'
				)
			)
		) );
