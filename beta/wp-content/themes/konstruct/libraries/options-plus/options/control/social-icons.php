		...
		?>
			...
			<ul class="icons">
				...
				<?php foreach ( $order as $id ):
					...
					?>
					<li class="item" data-id="<?php linethemes_esc_attr( $id ) ?>" <?php echo $link ?> data-title="<?php linethemes_esc_attr( $params['title'] ) ?>">
						<i class="fa <?php linethemes_esc_attr( $params['icon_class'] ) ?>"></i>
					</li>
				<?php endforeach ?>
			</ul>
			<input type="hidden" name="op-options[<?php linethemes_esc_attr( $this->id ) ?>]" value="<?php linethemes_esc_attr( json_encode( $this->value() ) ) ?>" />
		<?php
	}
}
