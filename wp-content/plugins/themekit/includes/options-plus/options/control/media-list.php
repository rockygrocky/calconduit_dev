...
class MediaList extends \OptionsPlus\Options\Control
{
	...
	public function render_content() {
		...
		if ( in_array( 'image', $this->media_types ) )
			$buttons[] = sprintf( '<a href="#" class="add-image">%s</a>', esc_html__( 'Insert Image(s)', 'option-plus' ) );

		if ( in_array( 'video', $this->media_types ) )
			$buttons[] = sprintf( '<a href="#" class="add-video">%s</a>', esc_html__( 'Insert Video(s)', 'option-plus' ) );
		?>
	...
	public static function templates() {
		$videoPane = new \OptionsPlus\Options\Container( array(
			...
			'sections'  => array(
				'all' => array(
					'title' => esc_html__( 'All', 'option-plus' )
				) 
			),
			'controls'  => array(
				'url' => array(
					'type'    => 'media-input',
					'section' => 'all',
					'label'   => esc_html__( 'URL', 'option-plus' ),
					'library' => 'video'
				)
			)
		) );
