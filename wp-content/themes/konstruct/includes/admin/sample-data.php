...
class Konstruct_SampleData
{
	...
	private function __construct() {
		...
		add_filter( 'konstruct/install_sample_data', array( $this, 'import_content' ), 10, 2 );
		add_filter( 'konstruct/install_sample_data', array( $this, 'update_metadata' ), 10, 2 );
		add_filter( 'konstruct/install_sample_data', array( $this, 'download_attachment' ), 10, 2 );
	}
	...
	public function admin_menu() {
		add_theme_page( __( 'Sample Data Installation', 'konstruct' ),
						__( 'Sample Data', 'konstruct' ),
	...
	public function admin_page() {
		?>
			...
			<div id="sample-data-installer">
				<h1><?php _e( 'Sample Data Installation', 'konstruct' ) ?></h1>

				<div class="start-screen">
					<p><?php _e( 'There is following tasks will be run for install sample data:', 'konstruct' ) ?></p>
					<ol class="tasks">
						<li><?php _e( 'Import sample content', 'konstruct' ) ?></li>
						<li><?php _e( 'Download media files', 'konstruct' ) ?></li>
					</ol>

					<p>
						<button type="button" id="install-sample-data" class="button-primary"><?php _e( 'Install Sample Data', 'konstruct' ) ?></button>
					</p>
				</div>
				...
				<div class="running-screen">
					<ol class="tasks">
						<li data-task="import-content">
							<span><?php _e( 'Import sample content', 'konstruct' ) ?></span>
							<span class="spinner-text"></span>
						</li>
						<li data-task="download-attachments">
							<span><?php _e( 'Download media files', 'konstruct' ) ?></span>
					...
					<p class="finish-actions">
						<span><?php _e( 'Congratulation! Sample data has been installed successfully', 'konstruct' ) ?></span>
					</p>
				</div>
			</div>
	...
	public function enqueue( $page ) {
		if ( $page == 'appearance_page_sample-data' ) {
			wp_enqueue_style( 'konstruct-sample-data' );
			wp_enqueue_script( 'konstruct-sample-data' );

			wp_localize_script( 'konstruct-sample-data', '_sampleDataLocalization', array(
				'confirm_installation' => __( 'Attention!!! Your existing data will be removed when install sample data. Are you sure you want to install sample data?', 'konstruct' )
			) );

			wp_localize_script( 'konstruct-sample-data', '_sampleDataInfo', array(
				'siteURL' => site_url(),
	...
	public function invoke() {
		...
		if ( isset( $_POST['step'] ) &&
			...
			try {
				$response = apply_filters( 'konstruct/install_sample_data', array( 'status' => 'success' ), $_POST );
	...
	public function import_content( $response, $context ) {
		...
		if ( $context['step'] == 'import-content' && wp_verify_nonce( $context['nonce'], 'sample_data_installation' ) ) {
			...
			if ( ! is_file( $sample_data_path ) )
				throw new Exception( sprintf( __( 'Sample data file not found: %s', 'konstruct' ), $sample_data_path ) );

			$file_handle = fopen( $sample_data_path, 'r' );
